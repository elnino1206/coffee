...
const selectedShip = computed(() => props.ship_methods.find((m) => m.value === ship.value));
        ...
        <Form action="/checkout" method="post" class="checkout" v-slot="{ errors, processing }">
            <div class="stack">
                <section class="panel">
                    ...
                    <div class="pay-options" style="margin-top: 12px">
                        ...
                        >
                            <input type="radio" name="ship_method" :value="method.value" v-model="ship" />
                            <span>
                                <strong>{{ method.label }}</strong><br />
                                <span class="tiny">
                                    {{ method.note }} ·
                                    {{ method.price ? formatPrice(method.price) : 'бесплатно' }}
                                </span>
                            </span>
                        </label>
                    </div>
                ...
                <section class="panel">
                    ...
                    <div class="form-grid" style="margin-top: 12px">
                        <label class="label">
                            <span class="label__text">Телефон <span class="req" aria-hidden="true">*</span></span>
                            <input class="field" name="phone" type="tel" autocomplete="tel" placeholder="+7 9XX XXX-XX-XX" required />
                        ...
                        <label class="label">
                            Email (необязательно)
                            <input class="field" name="email" type="email" placeholder="you@email.ru" />
                            <InputError :message="errors.email" />
                        </label>
                        <label class="label full" v-if="selectedShip?.requires_address">
                            Адрес доставки
                            <input class="field" name="address" placeholder="Город, улица, дом, квартира" />
                            <InputError :message="errors.address" />
                        </label>
                        <label class="label full">
                            Комментарий
                            <textarea class="textarea" name="comment" placeholder="Домофон, пожелания к доставке"></textarea>
                        </label>
                    </div>
                </section>
                ...
                <section class="panel">
                    ...
                    <div class="pay-options" style="margin-top: 12px">
                        ...
                        >
                            <input type="radio" name="pay_method" :value="method.value" v-model="pay" />
                            <span>
                                <strong>{{ method.label }}</strong><br />
                    ...
                    <p class="tiny" style="margin-top: 8px">
                        Списание подключается следующим шагом: сейчас заказ принимается
                        без оплаты.
                    </p>
                </section>
            </div>
            ...
            <aside class="summary">
                ...
                <div class="totals">
                    ...
                    <div class="grand"><span>Итого</span><span>{{ formatPrice(total) }}</span></div>
                </div>
                <label class="checkbox" style="margin: 12px 0">
                    <input type="checkbox" name="agreement" value="1" required />
                    <span>Согласен с офертой и политикой конфиденциальности</span>
                </label>
                <InputError :message="errors.agreement" />
                <button class="btn btn--petrol btn--block" type="submit" :disabled="processing">
                    Оформить заказ
                </button>
                <p class="tiny">
                    Все данные защищены и используются только для оформления заказа.
                </p>
            </aside>
        </Form>
    </div>
